...
Analyze changes in beta diversity from baseline to a single follow-up time point. This plotting function is designed for paired data with one baseline and one follow-up per subject, so we demonstrate it on the paired example dataset:

```{r beta-change}
data(subset_pairs.obj)

dist.change.obj <- mStat_calculate_beta_diversity(
  data.obj = subset_pairs.obj,
  dist.name = "BC"
)

pc.change.obj <- mStat_calculate_PC(
  dist.obj = dist.change.obj,
  method = "mds",
  k = 2,
  dist.name = "BC"
)
...
### Longitudinal Association Testing

```{r taxa-association}
# Test longitudinal associations at the genus level
...
# View the top associations for the delivery comparison
head(assoc.test$Genus[[1]], 10)
```

### Temporal Trend Testing

```{r taxa-trend}
# Test taxa-specific temporal trends
trend.taxa.test <- generate_taxa_trend_test_long(
  data.obj = ecam.obj,
  subject.var = "subject.id",
  time.var = "month_num",
  group.var = "delivery",
...
# View the top taxa with group-associated temporal trends
head(trend.taxa.test$Genus[[1]], 10)
```
...
```{r time-varying}
# Check whether candidate adjustment variables vary within subjects
time.varying <- mStat_identify_time_varying_vars(
  meta.dat = ecam.obj$meta.dat,
  adj.vars = c("delivery", "diet", "sex"),
  subject.var = "subject.id"
)
