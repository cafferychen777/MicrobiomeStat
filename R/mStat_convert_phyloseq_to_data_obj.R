...
mStat_convert_phyloseq_to_data_obj <- function (phylo.obj) {
  ...
  if (!is.null(phylo.obj@otu_table)) {
    otu_matrix <- as.matrix(as.data.frame(phylo.obj@otu_table))
  ...
  if (!is.null(phylo.obj@tax_table)) {
    data.obj$feature.ann <- as.matrix(as.data.frame(phylo.obj@tax_table))
  ...
  if (!is.null(phylo.obj@phy_tree)) {
    ...
    if (!ape::is.rooted(data.obj$tree)) {
      if (!requireNamespace("phangorn", quietly = TRUE)) {
        stop("Package 'phangorn' is required to midpoint-root an unrooted tree. Please install it or provide a rooted tree.")
      }
      message('Root the tree by midpointing ...')
      data.obj$tree <- phangorn::midpoint(data.obj$tree)
    }
