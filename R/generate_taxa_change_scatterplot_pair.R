...
#' data(peerj32)
#' data(peerj32.obj)
#' peerj32.obj$meta.dat <- peerj32.obj$meta.dat %>% select(all_of("subject")) %>% dplyr::distinct() %>%
#' mutate(cons = runif(dplyr::n(),0,5)) %>% dplyr::left_join(peerj32.obj$meta.dat,by = "subject") %>%
